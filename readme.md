...
## About this project
This project is an Dapp, allowiong users to upload pictures and their comments. Each user has a Metamask account, with an unique address, namely its public key. When a user uploads an image, its similarity with other images which have been uploaded will be checked and the user will be given a warning about the distance(the higher the more similar) between existing similar images.
